...
void MainThread() {
  ...
  eventListener = new EventListener(
      {XORSTR("cs_game_disconnected"), XORSTR("player_connect_full"),
       XORSTR("player_death"), XORSTR("item_purchase"), XORSTR("item_remove"),
       XORSTR("item_pickup"), XORSTR("player_hurt"), XORSTR("bomb_begindefuse"),
       XORSTR("enter_bombzone"), XORSTR("bomb_beginplant"),
       XORSTR("switch_team")});

  if (Hooker::HookRecvProp(XORSTR("CBaseViewModel"), XORSTR("m_nSequence"),
                           SkinChanger::sequenceHook))
    SkinChanger::sequenceHook->SetProxyFunction(
        (RecvVarProxyFn)SkinChanger::SetViewModelSequence);
...
void __attribute__((destructor)) Shutdown() {
  ...
  *s_bOverridePostProcessingDisable = false;
  ...
  cvar->ConsoleColorPrintf(ColorRGBA(255, 0, 0),
                           XORSTR("Fuzion Unloaded successfully.\n"));
}
