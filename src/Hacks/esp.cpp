...
HealthColorVar Settings::ESP::Glow::enemyVisibleColor =
    ImColor(255, 255, 0, 255);
HealthColorVar Settings::ESP::Glow::localplayerColor =
...
VMatrix vMatrix = {69.0f, 69.0f, 69.0f, 69.0f, 69.0f, 69.0f, 69.0f, 69.0f,
                   69.0f, 69.0f, 69.0f, 69.0f, 69.0f, 69.0f, 69.0f, 69.0f};
...
bool dzShouldDraw(C_BaseEntity* ent,
                  C_BasePlayer* localplayer)  // Ghetto way to fix a CTD.
{
  if (!localplayer || !ent || !localplayer->GetAlive()) return false;
  return !(Settings::ESP::DangerZone::drawDistEnabled &&
           (localplayer->GetVecOrigin().DistTo(ent->GetVecOrigin()) >
            Settings::ESP::DangerZone::drawDist));
}

static void CheckActiveSounds() {
  static CUtlVector<SndInfo_t> sounds;  // this has to be static.
  char buf[PATH_MAX];
  static int lastSoundGuid = 0;  // the Unique sound playback ID's increment. It
                                 // does not get reset to 0
  sound->GetActiveSounds(sounds);
  for (int i = 0; i < sounds.Count(); i++) {
    if (sounds[i].m_nSoundSource <= 0 ||
        sounds[i].m_nSoundSource >
            63)  // environmental sounds or out of bounds.
      continue;
    if (!sounds[i].m_pOrigin)  // no location
      continue;
    if (sounds[i].m_nGuid <= lastSoundGuid)  // same sound we marked last time
      continue;
    if (sounds[i].m_nSoundSource == engine->GetLocalPlayer()) continue;
    ...
    if (strstr(buf, XORSTR("player/land")) != nullptr) {
      {
        ...
        playerFootsteps[sounds[i].m_nSoundSource].emplace_back(
            *sounds[i].m_pOrigin,
            (Util::GetEpochTime() + Settings::ESP::Sounds::time));
      }  // RAII mutex lock
  ...
  // GetActiveSounds allocates new memory to our CUtlVector every time for some
  // reason.
  sounds.m_Size = 0;  // Setting this to 0 makes it use the same memory each
                      // time instead of grabbing more.
}
...
static float GetArmourHealth(float flDamage, int ArmorValue) {
  ...
  if (flCurDamage == 0.0f || ArmorValue == 0) return flCurDamage;
...
static bool GetBox(C_BaseEntity* entity, int& x, int& y, int& w, int& h) {
  ...
  Vector flb, brt, blb, frt, frb, brb, blt,
      flt;  // think of these as Front-Left-Bottom/Front-Left-Top... Etc.
  ...
  Vector points[] = {Vector(min.x, min.y, min.z), Vector(min.x, max.y, min.z),
  ...
  if (debugOverlay->ScreenPosition(points[3], flb) ||
      debugOverlay->ScreenPosition(points[5], brt) ||
      debugOverlay->ScreenPosition(points[0], blb) ||
      debugOverlay->ScreenPosition(points[4], frt) ||
      debugOverlay->ScreenPosition(points[2], frb) ||
      debugOverlay->ScreenPosition(points[1], brb) ||
      debugOverlay->ScreenPosition(points[6], blt) ||
      debugOverlay->ScreenPosition(points[7], flt))
    return false;
...
ImColor ESP::GetESPPlayerColor(C_BasePlayer* player, bool visible) {
  ...
  if (!localplayer) return ImColor(255, 255, 255, 255);
...
bool ESP::WorldToScreen(const Vector& origin, ImVec2* const screen) {
  float w = vMatrix[3][0] * origin.x + vMatrix[3][1] * origin.y +
            vMatrix[3][2] * origin.z + vMatrix[3][3];
  ...
  screen->x =
      halfWidth + (0.5f *
                       ((vMatrix[0][0] * origin.x + vMatrix[0][1] * origin.y +
                         vMatrix[0][2] * origin.z + vMatrix[0][3]) *
                        inverseW) *
                       width +
                   0.5f);
  ...
  screen->y =
      halfHeight - (0.5f *
                        ((vMatrix[1][0] * origin.x + vMatrix[1][1] * origin.y +
                          vMatrix[1][2] * origin.z + vMatrix[1][3]) *
                         inverseW) *
                        height +
                    0.5f);
  return true;
...
static void DrawBox(ImColor color, int x, int y, int w, int h,
                    C_BaseEntity* entity) {
  if (Settings::ESP::Boxes::type == BoxType::FRAME_2D) {
    ...
    Draw::AddRect(x + squareLine, y - 1, x + squareLine + 1, y + 2,
                  ImColor(10, 10, 10, 190));
    Draw::AddRect(x - 1, y + squareLine, x + 2, y + squareLine + 1,
                  ImColor(10, 10, 10, 190));
    ...
    Draw::AddRect(x + w - squareLine - 1, y - 1, x + w - squareLine, y + 2,
                  ImColor(10, 10, 10, 190));
    Draw::AddRect(x + w - 2, y + squareLine, x + w + 1, y + squareLine + 1,
                  ImColor(10, 10, 10, 190));
    ...
    Draw::AddRect(x + squareLine, y + h - 2, x + squareLine + 1, y + h + 1,
                  ImColor(10, 10, 10, 190));
    Draw::AddRect(x - 1, y + h - squareLine - 1, x + 2, y + h - squareLine,
                  ImColor(10, 10, 10, 190));
    ...
    Draw::AddRect(x + w - squareLine, y + h - 2, x + w - squareLine + 1,
                  y + h + 1, ImColor(10, 10, 10, 190));
    Draw::AddRect(x + w - 2, y + h - squareLine - 1, x + w + 1,
                  y + h - squareLine, ImColor(10, 10, 10, 190));
    ...
    Draw::AddRectFilled(x, y, x + w, y + h,
                        ImColor(color.Value.x, color.Value.y, color.Value.z,
                                21 * (1.0f / 255.0f)));
...
haven't drawn this player yet playerDrawTimes[entity->GetIndex()] =
Util::GetEpochTime();
}
...
if ( diffTime >= 12 ) {
  for ( int i = 0; i < set->numhitboxes; i++ ) {
      ...
      debugOverlay->DrawPill( vMin, vMax, hitbox->radius, color.r, color.g,
color.b, 0.025f, color.a, false );
  }
...
static void DrawEntity(C_BaseEntity* entity, const char* string,
                       ImColor color) {
  int x, y, w, h;
  ...
  Draw::AddText((int)(x + (w / 2) - (nameSize.x / 2)), y + h + 2, string,
                color);
}
static void DrawSkeleton(C_BasePlayer* player, C_BasePlayer* localplayer) {
  ...
  if (!player->SetupBones(pBoneToWorldOut, 128, 256, 0)) return;
  ...
  for (int i = 0; i < pStudioModel->numbones; i++) {
    ...
    Vector vBonePos1;
    if (debugOverlay->ScreenPosition(
            Vector(pBoneToWorldOut[i][0][3], pBoneToWorldOut[i][1][3],
                   pBoneToWorldOut[i][2][3]),
            vBonePos1))
      continue;

    Vector vBonePos2;
    if (debugOverlay->ScreenPosition(
            Vector(pBoneToWorldOut[pBone->parent][0][3],
                   pBoneToWorldOut[pBone->parent][1][3],
                   pBoneToWorldOut[pBone->parent][2][3]),
            vBonePos2))
      continue;

    Draw::AddLine(vBonePos1.x, vBonePos1.y, vBonePos2.x, vBonePos2.y,
                  Entity::IsTeamMate(player, localplayer)
                      ? Settings::ESP::Skeleton::allyColor.Color()
                      : Settings::ESP::Skeleton::enemyColor.Color());
  }
}

static void DrawBackTrack(C_BasePlayer* player, bool draw_headdot = true,
                          bool draw_skeleton = false) {
  Vector head2D;

  for (auto&& tick : BackTrack::backtrack_frames) {
    for (auto&& record : tick.records) {
      if (debugOverlay->ScreenPosition(record.head, head2D)) continue;
      Draw::AddCircleFilled(head2D.x, head2D.y, Settings::ESP::HeadDot::size,
                            ESP::GetESPPlayerColor(player, false), 3);
...
static void DrawBulletTrace(C_BasePlayer* player) {
  ...
  if (debugOverlay->ScreenPosition(src3D, src) ||
      debugOverlay->ScreenPosition(tr.endpos, dst))
    return;

  Draw::AddLine(src.x, src.y, dst.x, dst.y,
                ESP::GetESPPlayerColor(player, true));
  Draw::AddRectFilled((int)(dst.x - 3), (int)(dst.y - 3), 6, 6,
                      ESP::GetESPPlayerColor(player, false));
}
static void DrawTracer(C_BasePlayer* player) {
  ...
  if (debugOverlay->ScreenPosition(src3D, src)) return;
  ...
  bool bIsVisible = Entity::IsVisible(player, (int)Bone::BONE_HEAD, 180.f,
                                      Settings::ESP::Filters::smokeCheck);
  Draw::AddLine((int)(src.x), (int)(src.y), x, y,
                ESP::GetESPPlayerColor(player, bIsVisible));
}
static void DrawAimbotSpot() {
  ...
  if (Settings::Debug::AutoAim::target.IsZero()) return;

  Vector spot2D;
  if (debugOverlay->ScreenPosition(Settings::Debug::AutoAim::target, spot2D))
    return;

  Draw::AddLine(Paint::engineWidth / 2, Paint::engineHeight / 2, spot2D.x,
                spot2D.y, ImColor(45, 235, 60));
  Draw::AddCircle(Paint::engineWidth / 2, Paint::engineHeight / 2, 1,
                  ImColor(45, 235, 60));
  ...
  if (!debugOverlay->ScreenPosition(lastRayEnd, end2D)) {
    Draw::AddLine(start2D.x, start2D.y, end2D.x, end2D.y,
                  ImColor(255, 25, 25, 255));
  }
}
static void DrawBoneMap(C_BasePlayer* player) {
  ...
  for (int i = 1; i < pStudioModel->numbones; i++) {
    ...
    if (debugOverlay->ScreenPosition(bone3D, bone2D)) continue;
    if (Settings::Debug::BoneMap::justDrawDots) {
      Draw::AddCircleFilled(bone2D.x, bone2D.y, 2.0f, ImColor(255, 0, 255, 255),
                            10);
    } else {
      char buffer[32];
  ...
  cvar->ConsoleDPrintf(XORSTR("(%s)-ModelName: %s, numBones: %d\n"),
                       entityInformation.name, pStudioModel->name,
                       pStudioModel->numbones);
}
static void DrawAutoWall(C_BasePlayer* player) {
  ...
  for (int i = 0; i < 31; i++) {
    ...
    int damage =
        (int)Autowall::GetDamage(bone3D, !Settings::Aimbot::friendly, data);
    char buffer[4];
  ...
  if (!player->SetupBones(matrix, 128, 0x100, 0.f)) return;
  ...
  mstudiobbox_t* bbox =
      hdr->pHitbox((int)Hitbox::HITBOX_HEAD, 0);  // bounding box
  if (!bbox) return;
  ...
  // 0 - center, 1 - forehead, 2 - skullcap, 3 - upperleftear, 4 -
  // upperrightear, 5 - uppernose, 6 - upperbackofhead 7 - leftear, 8 -
  // rightear, 9 - nose, 10 - backofhead
  Vector headPoints[11] = {center, center, center, center, center, center,
  ...
  Autowall::FireBulletData data;
  for (int i = 0; i < 11; i++) {
    int damage = (int)Autowall::GetDamage(headPoints[i],
                                          !Settings::Aimbot::friendly, data);
    char buffer[4];
    snprintf(buffer, sizeof(buffer), "%d", damage);

    Vector string2D;
    if (debugOverlay->ScreenPosition(headPoints[i], string2D)) continue;
...
static void DrawHeaddot(C_BasePlayer* player) {
  ...
  if (debugOverlay->ScreenPosition(Vector(head3D.x, head3D.y, head3D.z),
                                   head2D))
    return;

  bool bIsVisible = false;
  if (Settings::ESP::Filters::visibilityCheck || Settings::ESP::Filters::legit)
    bIsVisible = Entity::IsVisible(player, (int)Bone::BONE_HEAD, 180.f,
                                   Settings::ESP::Filters::smokeCheck);

  Draw::AddCircleFilled(head2D.x, head2D.y, Settings::ESP::HeadDot::size,
                        ESP::GetESPPlayerColor(player, bIsVisible), 10);
}
...
static void DrawSounds(C_BasePlayer* player, ImColor playerColor) {
  ...
  if (lock.owns_lock()) {
    if (playerFootsteps[player->GetIndex()].empty()) return;

    for (auto it = playerFootsteps[player->GetIndex()].begin();
         it != playerFootsteps[player->GetIndex()].end(); it++) {
      long diff = it->expiration - Util::GetEpochTime();

      if (diff <= 0) {
        playerFootsteps[player->GetIndex()]
            .pop_front();  // This works because footsteps are a trail.
        continue;
      }

      float percent = (float)diff / (float)Settings::ESP::Sounds::time;
      Color drawColor = Color::FromImColor(playerColor);
      drawColor.a = std::min(powf(percent * 2, 0.6f), 1.f) *
                    drawColor.a;  // fades out alpha when its below 0.5

      float circleRadius = fabs(percent - 1.0f) * 42.0f;
      float points = std::max(12.0f, circleRadius * 0.75f);

      Draw::AddCircle3D(
          it->position, circleRadius,
          ImColor(drawColor.r, drawColor.g, drawColor.b, drawColor.a),
          (int)points);
    }
...
static void DrawPlayerHealthBars(C_BasePlayer* player, int x, int y, int w,
                                 int h, ImColor color) {
  ...
  if (Settings::ESP::Bars::colorType == BarColorType::HEALTH_BASED)
    barColor = ImColor(std::min(510 * (100 - healthValue) / 100, 255),
                       std::min(510 * healthValue / 100, 255), 25, 255);
  ...
  if (Settings::ESP::Bars::type == BarType::VERTICAL) {
    ...
    barx -= barw + boxSpacing;  // spacing(1px) + outline(1px) + bar(2px) +
                                // outline (1px) = 8 px
    ...
    bary += barh + boxSpacing;  // player box(?px) + spacing(1px) + outline(1px)
                                // + bar(2px) + outline (1px) = 5 px
    ...
    if (HealthPerc > 0) {
      barw *= HealthPerc;
      Draw::AddRect(barx + 1, bary + 1, barx + barw - 1, bary + barh - 1,
                    barColor);
    }
    barsSpacing.y += barh;
  } else if (Settings::ESP::Bars::type == BarType::HORIZONTAL_UP) {
    barh = 4;                   // outline(1px) + bar(2px) + outline(1px) = 4px;
    bary -= barh + boxSpacing;  // spacing(1px) + outline(1px) + bar(2px) +
                                // outline (1px) = 5 px

    Draw::AddRect(barx - 1, bary - 1, barx + barw + 1, bary + barh + 1,
                  ImColor(255, 255, 255, 170));
    Draw::AddRectFilled(barx, bary, barx + barw, bary + barh,
                        ImColor(10, 10, 10, 255));
...
static void DrawPlayerText(C_BasePlayer* player, C_BasePlayer* localplayer,
                           int x, int y, int w, int h) {
  int boxSpacing = Settings::ESP::Boxes::enabled ? 3 : 0;
  int lineNum = 1;
  int nameOffset = (int)(Settings::ESP::Bars::type == BarType::HORIZONTAL_UP
                             ? boxSpacing + barsSpacing.y
                             : 0);
  ...
  if (Settings::ESP::Info::name || Settings::ESP::Info::clan) {
    ...
    if (Settings::ESP::Info::clan)
      displayString +=
          std::string((*csPlayerResource)->GetClan(player->GetIndex()));

    if (Settings::ESP::Info::clan && Settings::ESP::Info::name)
      displayString += " ";

    if (Settings::ESP::Info::name) displayString += playerInfo.name;

    Vector2D nameSize = Draw::GetTextSize(displayString.c_str(), esp_font);
    Draw::AddText(x + (w / 2) - (nameSize.x / 2), (y - textSize.y - nameOffset),
                  displayString.c_str(),
                  Entity::IsTeamMate(player, localplayer)
                      ? Settings::ESP::allyInfoColor.Color()
  ...
  if (Settings::ESP::Info::steamId) {
    ...
    Draw::AddText((x + (w / 2) - (rankSize.x / 2)),
                  (y - (textSize.y * lineNum) - nameOffset), playerInfo.guid,
                  Entity::IsTeamMate(player, localplayer)
                      ? Settings::ESP::allyInfoColor.Color()
                      : Settings::ESP::enemyInfoColor.Color());
    lineNum++;
  ...
  if (Settings::ESP::Info::rank) {
    ...
    if (rank >= 0 && rank < 19) {
      ...
      Draw::AddText((x + (w / 2) - (rankSize.x / 2)),
                    (y - (textSize.y * lineNum) - nameOffset), ESP::ranks[rank],
                    Entity::IsTeamMate(player, localplayer)
                        ? Settings::ESP::allyInfoColor.Color()
                        : Settings::ESP::enemyInfoColor.Color());
    }
  ...
  if (Settings::ESP::Info::health) {
    ...
    Draw::AddText(x + w + boxSpacing, (y + h - textSize.y), buf.c_str(),
                  Entity::IsTeamMate(player, localplayer)
                      ? Settings::ESP::allyInfoColor.Color()
                      : Settings::ESP::enemyInfoColor.Color());
  }
  ...
  if (Settings::ESP::Info::armor) {
    std::string buf = std::to_string(player->GetArmor()) +
                      (player->HasHelmet() ? XORSTR(" AP*") : XORSTR(" AP"));
    Draw::AddText(x + w + boxSpacing, (y + h - (textSize.y / 3)), buf.c_str(),
                  Entity::IsTeamMate(player, localplayer)
                      ? Settings::ESP::allyInfoColor.Color()
                      : Settings::ESP::enemyInfoColor.Color());
  }
  ...
  if (Settings::ESP::Info::weapon && activeWeapon) {
    std::string modelName = Util::Items::GetItemDisplayName(
        *activeWeapon->GetItemDefinitionIndex());
    int offset = (int)(Settings::ESP::Bars::type == BarType::HORIZONTAL ||
                               Settings::ESP::Bars::type == BarType::INTERWEBZ
                           ? boxSpacing + barsSpacing.y + 1
                           : 0);

    Vector2D weaponTextSize = Draw::GetTextSize(modelName.c_str(), esp_font);
    Draw::AddText((x + (w / 2) - (weaponTextSize.x / 2)), y + h + offset,
                  modelName.c_str(),
                  Entity::IsTeamMate(player, localplayer)
                      ? Settings::ESP::allyInfoColor.Color()
                      : Settings::ESP::enemyInfoColor.Color());
  ...
  if (Settings::ESP::Info::reloading) {
    ...
    for (int i = 0; i <= layers->Count(); i++) {
      if (player->GetSequenceActivity(layers->operator[](i).m_nSequence) ==
              (int)CCSGOAnimStatePoses::ACT_CSGO_RELOAD &&
          layers->operator[](i).m_flWeight != 0.f)
  ...
  if (Settings::ESP::Info::planting &&
      player->GetIndex() == (*csPlayerResource)->GetPlayerC4())
  ...
  if (Settings::Debug::AnimLayers::draw) {
    CUtlVector<AnimationLayer>* layers = player->GetAnimOverlay();
    for (int i = 0; i <= layers->Count(); i++) {
      stringsToShow.push_back(Util::GetActivityName(
          player->GetSequenceActivity(layers->operator[](i).m_nSequence)));
    }
  }

  for (unsigned int i = 0; i < stringsToShow.size(); i++) {
    Draw::AddText(x + w + boxSpacing, (y + (i * (textSize.y + 2))),
                  stringsToShow[i].c_str(),
                  Entity::IsTeamMate(player, localplayer)
                      ? Settings::ESP::allyInfoColor.Color()
                      : Settings::ESP::enemyInfoColor.Color());
  }
...
static void DrawPlayer(C_BasePlayer* player) {
  ...
  if (player == localplayer && !Settings::ESP::Filters::localplayer) return;

  if (!Entity::IsTeamMate(player, localplayer) &&
      !Settings::ESP::Filters::enemies)
    return;

  if (player != localplayer && Entity::IsTeamMate(player, localplayer) &&
      !Settings::ESP::Filters::allies)
    return;
  ...
  if (Settings::ESP::Filters::visibilityCheck ||
    ...
    bIsVisible = Entity::IsVisible(player, (int)Bone::BONE_HEAD, 180.f,
                                   Settings::ESP::Filters::smokeCheck);
    if (!bIsVisible && Settings::ESP::Filters::legit) return;
  }

  ImColor playerColor = ESP::GetESPPlayerColor(player, bIsVisible);

  int x, y, w, h;
  if (!GetBox(player, x, y, w, h)) return;

  if (Settings::ESP::Boxes::enabled) DrawBox(playerColor, x, y, w, h, player);

  if (Settings::ESP::Sprite::enabled) DrawSprite(x, y, w, h, player);

  if (Settings::ESP::Bars::enabled)
    DrawPlayerHealthBars(player, x, y, w, h, playerColor);

  if (Settings::ESP::Skeleton::enabled) DrawSkeleton(player, localplayer);

  if (Settings::ESP::BulletTracers::enabled) DrawBulletTrace(player);
  ...
  if (Settings::Debug::AutoAim::drawTarget) DrawAimbotSpot();
...
static void DrawBomb(C_BaseCombatWeapon* bomb, C_BasePlayer* localplayer) {
  if (!(*csGameRules) || !(*csGameRules)->IsBombDropped()) return;

  // Draw Distance only in DangerZone
  if (Util::IsDangerZone() && !dzShouldDraw(bomb, localplayer)) return;
...
static void DrawPlantedBomb(C_PlantedC4* bomb, C_BasePlayer* localplayer) {
  ...
  ImColor color = bomb->GetBombDefuser() != -1 || bomb->IsBombDefused()
                      ? Settings::ESP::bombDefusingColor.Color()
                      : Settings::ESP::bombColor.Color();
  ...
  if (bomb->IsBombDefused() || !bomb->IsBombTicking() || bombTimer <= 0.f) {
    ...
    float damage = std::max(
        (int)ceilf(GetArmourHealth(flDamage, localplayer->GetArmor())), 0);

    displayText << XORSTR("Bomb: ") << std::fixed << std::showpoint
                << std::setprecision(1) << bombTimer << XORSTR(", damage: ")
                << (int)damage;
  }
...
static void DrawDefuseKit(C_BaseEntity* defuser, C_BasePlayer* localplayer) {
  // Draw Distance only in DangerZone
  if (Util::IsDangerZone() && !dzShouldDraw(defuser, localplayer)) return;
...
static void DrawDroppedWeapons(C_BaseCombatWeapon* weapon,
                               C_BasePlayer* localplayer) {
  // Draw Distance only in DangerZone
  if (Util::IsDangerZone() && !dzShouldDraw(weapon, localplayer)) return;

  Vector vOrig = weapon->GetVecOrigin();
  int owner = weapon->GetOwner();

  if (owner > -1 || (vOrig.x == 0 && vOrig.y == 0 && vOrig.z == 0)) return;

  std::string modelName =
      Util::Items::GetItemDisplayName(*weapon->GetItemDefinitionIndex());
...
static void DrawHostage(C_BaseEntity* hostage, C_BasePlayer* localplayer) {
  // Draw Distance only in DangerZone
  if (Util::IsDangerZone() && !dzShouldDraw(hostage, localplayer)) return;
...
static void DrawSafe(C_BaseEntity* safe, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(safe, localplayer)) return;
  if (*(bool*)((uintptr_t)safe + offsets.DT_BRC4Target.m_bBrokenOpen)) return;
  DrawEntity(safe, XORSTR("Safe"),
             Settings::ESP::DangerZone::safeColor.Color());
}

static void DrawAmmoBox(C_BaseEntity* ammobox, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(ammobox, localplayer)) return;
  DrawEntity(ammobox, XORSTR("Ammo box"),
             Settings::ESP::DangerZone::ammoboxColor.Color());
}

static void DrawSentryTurret(C_BaseEntity* sentry, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(sentry, localplayer)) return;

  std::string name = XORSTR("Sentry Turret");
  name += XORSTR(" | ");
  name += std::to_string(
      *(int*)((uintptr_t)sentry + offsets.DT_Dronegun.m_iHealth));
  name += XORSTR(" HP");
  DrawEntity(sentry, name.c_str(),
             Settings::ESP::DangerZone::dronegunColor.Color());
}

static void DrawRadarJammer(C_BaseEntity* jammer, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(jammer, localplayer)) return;
  DrawEntity(jammer, XORSTR("Radar Jammer"),
             Settings::ESP::DangerZone::radarjammerColor.Color());
}

static void DrawExplosiveBarrel(C_BaseEntity* barrel,
                                C_BasePlayer* localplayer) {
  if (!dzShouldDraw(barrel, localplayer)) return;
  studiohdr_t* barrelModel = modelInfo->GetStudioModel(barrel->GetModel());
  if (!barrelModel) return;
  std::string mdlName = barrelModel->name;
  mdlName = mdlName.substr(mdlName.find_last_of('/') + 1);
  if (mdlName.find(XORSTR("exploding_barrel.mdl")) == mdlName.npos) return;
  DrawEntity(barrel, XORSTR("Explosive Barrel"),
             Settings::ESP::DangerZone::barrelColor.Color());
}

static void DrawDrone(C_BaseEntity* drone, C_BasePlayer* localplayer) {
  // TODO: Add Drone info (owner/package).
  if (!dzShouldDraw(drone, localplayer)) return;
  DrawEntity(drone, XORSTR("Drone"),
             Settings::ESP::DangerZone::droneColor.Color());
}

static void DrawCash(C_BaseEntity* cash, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(cash, localplayer)) return;
  DrawEntity(cash, XORSTR("Cash"),
             Settings::ESP::DangerZone::cashColor.Color());
}

static void DrawTablet(C_BaseEntity* tablet, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(tablet, localplayer)) return;
  DrawEntity(tablet, XORSTR("Tablet"),
             Settings::ESP::DangerZone::tabletColor.Color());
}

static void DrawHealthshot(C_BaseEntity* healthshot,
                           C_BasePlayer* localplayer) {
  if (!dzShouldDraw(healthshot, localplayer)) return;
  DrawEntity(healthshot, XORSTR("Healthshot"),
             Settings::ESP::DangerZone::healthshotColor.Color());
}

static void DrawLootCrate(C_BaseEntity* crate, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(crate, localplayer)) return;
  ...
  crateName += XORSTR(" | ");
  crateName += std::to_string(
      *(int*)((uintptr_t)crate + offsets.DT_PhysPropLootCrate.m_iHealth));
  crateName += XORSTR(" HP");

  DrawEntity(crate, crateName.c_str(),
             Settings::ESP::DangerZone::lootcrateColor.Color());
}

static void DrawMelee(C_BaseCombatWeapon* weapon, C_BasePlayer* localplayer) {
  if (!weapon) return;

  if (!dzShouldDraw(weapon, localplayer)) return;

  std::string modelName =
      Util::Items::GetItemDisplayName(*weapon->GetItemDefinitionIndex());
  DrawEntity(weapon, modelName.c_str(),
             Settings::ESP::DangerZone::meleeColor.Color());
}

static void DrawDZItems(C_BaseEntity* item, C_BasePlayer* localplayer) {
  if (!dzShouldDraw(item, localplayer)) return;
  ...
  else if (mdlName.find(XORSTR("exojump")) !=
           mdlName.npos)  // TODO: not working.
    itemName = XORSTR("Exojump");
  else
    itemName = mdlName;

  DrawEntity(item, itemName.c_str(),
             Settings::ESP::DangerZone::upgradeColor.Color());
}

static void DrawThrowable(C_BaseEntity* throwable, ClientClass* client,
                          C_BasePlayer* localplayer) {
  // Draw Distance only in DangerZone
  if (Util::IsDangerZone() && !dzShouldDraw(throwable, localplayer)) return;
  ...
  if (!strstr(hdr->name, XORSTR("thrown")) &&
      !strstr(hdr->name, XORSTR("dropped")))
    return;
...
static void DrawGlow() {
  ...
  for (int i = 0; i < glowManager->m_GlowObjectDefinitions.Count(); i++) {
    GlowObjectDefinition_t& glow_object =
        glowManager->m_GlowObjectDefinitions[i];

    if (glow_object.IsUnused() || !glow_object.m_pEntity) continue;
    ...
    if (client->m_ClassID == EClassIds::CCSPlayer) {
      ...
      if (player->GetDormant() || !player->GetAlive()) continue;
      ...
      if (player == localplayer) {
        ...
        if (!Entity::IsTeamMate(player, localplayer)) {
          ...
          color = Settings::ESP::Glow::allyColor.Color(player);
               ...
               (strstr(client->m_pNetworkName, XORSTR("Weapon")) ||
                client->m_ClassID == EClassIds::CDEagle ||
                client->m_ClassID == EClassIds::CAK47)) {
      color = Settings::ESP::Glow::weaponColor.Color();
    } else if (client->m_ClassID == EClassIds::CBaseCSGrenadeProjectile ||
               client->m_ClassID == EClassIds::CDecoyProjectile ||
               client->m_ClassID == EClassIds::CMolotovProjectile ||
               client->m_ClassID == EClassIds::CSmokeGrenadeProjectile) {
      ...
      if (localplayer->HasDefuser() ||
          localplayer->GetTeam() == TeamID::TEAM_TERRORIST)
        shouldGlow = false;
    } else if (client->m_ClassID == EClassIds::CChicken) {
      color = Settings::ESP::Glow::chickenColor.Color();

      *reinterpret_cast<C_Chicken*>(glow_object.m_pEntity)->GetShouldGlow() =
          shouldGlow;
    }
...
static void DrawFOVCrosshair() {
  if (!Settings::ESP::FOVCrosshair::enabled) return;

  C_BasePlayer* localplayer =
      (C_BasePlayer*)entityList->GetClientEntity(engine->GetLocalPlayer());
  if (!localplayer->GetAlive()) return;

  if (Settings::Aimbot::AutoAim::fov > OverrideView::currentFOV) return;
  ...
  if (Settings::Aimbot::AutoAim::realDistance) {
    ...
    Vector max2D;
    if (debugOverlay->ScreenPosition(maxAimAt, max2D)) return;

    radius = fabsf(Paint::engineWidth / 2 - max2D.x);
  } else
    radius = ((Settings::Aimbot::AutoAim::fov / OverrideView::currentFOV) *
              Paint::engineWidth) /
             2;

  radius = std::min(radius,
                    (((180.f / OverrideView::currentFOV) * Paint::engineWidth) /
                     2));  // prevents a big radius (CTD).

  if (Settings::ESP::FOVCrosshair::filled)
    Draw::AddCircleFilled(Paint::engineWidth / 2, Paint::engineHeight / 2,
                          radius, Settings::ESP::FOVCrosshair::color.Color(),
                          std::max(12, (int)radius * 2));
  else
    Draw::AddCircle(Paint::engineWidth / 2, Paint::engineHeight / 2, radius,
                    Settings::ESP::FOVCrosshair::color.Color(),
                    std::max(12, (int)radius * 2));
}
...
static void DrawSpread() {
  ...
  if (Settings::ESP::Spread::enabled) {
    ...
    if (cone > 0.0f) {
      ...
      Draw::AddRect(((Paint::engineWidth / 2) - radius),
                    (Paint::engineHeight / 2) - radius + 1,
                    (Paint::engineWidth / 2) + radius + 1,
                    (Paint::engineHeight / 2) + radius + 2,
...
static void DrawScope() {
  ...
  Draw::AddLine(0, Paint::engineHeight * 0.5, Paint::engineWidth,
                Paint::engineHeight * 0.5, ImColor(0, 0, 0, 255));
  Draw::AddLine(Paint::engineWidth * 0.5, 0, Paint::engineWidth * 0.5,
                Paint::engineHeight, ImColor(0, 0, 0, 255));
}

bool ESP::PrePaintTraverse(VPANEL vgui_panel, bool force_repaint,
                           bool allow_force) {
...
void ESP::Paint() {
  ...
  for (int i = 1; i < entityList->GetHighestEntityIndex(); i++) {
    ...
    if (client->m_ClassID == EClassIds::CCSPlayer &&
        (Settings::ESP::Filters::enemies || Settings::ESP::Filters::allies ||
    ...
    } else if (client->m_ClassID == EClassIds::CHostage &&
               Settings::ESP::Filters::hostages) {
      DrawHostage(entity, localplayer);
    } else if (client->m_ClassID == EClassIds::CBaseAnimating &&
               Settings::ESP::Filters::defusers) {
      DrawDefuseKit(entity, localplayer);
    } else if (client->m_ClassID == EClassIds::CChicken &&
               Settings::ESP::Filters::chickens) {
      DrawChicken(entity);
    } else if (client->m_ClassID == EClassIds::CFish &&
               Settings::ESP::Filters::fishes) {
      DrawFish(entity);
    } else if (Settings::ESP::Filters::throwables &&
               strstr(client->m_pNetworkName, XORSTR("Projectile"))) {
      DrawThrowable(entity, client, localplayer);
    } else if (Util::IsDangerZone()) {
      if (Settings::ESP::DangerZone::safe &&
          client->m_ClassID == EClassIds::CBRC4Target)
        DrawSafe(entity, localplayer);

      else if (Settings::ESP::DangerZone::dronegun &&
               client->m_ClassID == EClassIds::CDronegun)
        DrawSentryTurret(entity, localplayer);

      else if (Settings::ESP::DangerZone::ammobox &&
               client->m_ClassID == EClassIds::CPhysPropAmmoBox)
        DrawAmmoBox(entity, localplayer);

      else if (Settings::ESP::DangerZone::radarjammer &&
               client->m_ClassID == EClassIds::CPhysPropRadarJammer)
        DrawRadarJammer(entity, localplayer);

      else if (Settings::ESP::DangerZone::barrel &&
               client->m_ClassID == EClassIds::CPhysicsProp)
        DrawExplosiveBarrel(entity, localplayer);

      else if (Settings::ESP::DangerZone::drone &&
               client->m_ClassID == EClassIds::CDrone)
        DrawDrone(entity, localplayer);

      else if (Settings::ESP::DangerZone::cash &&
               client->m_ClassID == EClassIds::CItemCash)
        DrawCash(entity, localplayer);

      else if (Settings::ESP::DangerZone::tablet &&
               client->m_ClassID == EClassIds::CTablet)
        DrawTablet(entity, localplayer);

      else if (Settings::ESP::DangerZone::healthshot &&
               client->m_ClassID == EClassIds::CItem_Healthshot)
        DrawHealthshot(entity, localplayer);

      else if (Settings::ESP::DangerZone::lootcrate &&
               client->m_ClassID == EClassIds::CPhysPropLootCrate)
        DrawLootCrate(entity, localplayer);

      else if (Settings::ESP::DangerZone::melee &&
               (client->m_ClassID == EClassIds::CMelee ||
                client->m_ClassID == EClassIds::CKnife)) {
        C_BaseCombatWeapon* weapon = (C_BaseCombatWeapon*)entity;
        DrawMelee(weapon, localplayer);
      }

      else if (Settings::ESP::DangerZone::upgrade &&
               client->m_ClassID == EClassIds::CPhysPropWeaponUpgrade)
        DrawDZItems(entity, localplayer);
    }
  }

  if (Settings::ESP::FOVCrosshair::enabled) DrawFOVCrosshair();
  if (Settings::ESP::Spread::enabled || Settings::ESP::Spread::spreadLimit)
    DrawSpread();
  if (Settings::NoScopeBorder::enabled && localplayer->IsScoped()) DrawScope();
}
...
void ESP::DrawModelExecute() {
  ...
  if (Settings::ESP::Glow::enabled) DrawGlow();
}
...
void ESP::CreateMove(CUserCmd* cmd) {
  ...
  if (Settings::ESP::enabled && Settings::ESP::Sounds::enabled &&
      (Settings::ESP::Filters::allies || Settings::ESP::Filters::enemies ||
       Settings::ESP::Filters::localplayer)) {
    CheckActiveSounds();
  }
}
