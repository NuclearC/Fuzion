...
void ThirdPerson::OverrideView(CViewSetup* pSetup) {
  ...
  if (localplayer->GetAlive() && Settings::ThirdPerson::enabled &&
    ...
    Vector camOff =
        Vector(cos(DEG2RAD(viewAngles.y)) * Settings::ThirdPerson::distance,
    ...
    input->m_vecCameraOffset =
        Vector(viewAngles.x, viewAngles.y,
               Settings::ThirdPerson::distance *
...
void ThirdPerson::FrameStageNotify(ClientFrameStage_t stage) {
  if (stage == ClientFrameStage_t::FRAME_RENDER_START && engine->IsInGame()) {
    ...
    if (localplayer && localplayer->GetAlive() &&
        Settings::ThirdPerson::enabled && input->m_fCameraInThirdPerson) {
