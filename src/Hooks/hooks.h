...
void SetMouseCodeState(void* thisptr, ButtonCode_t code,
                       MouseCodeState_t state);

/* Material */
void OverrideConfig(void* thisptr, MaterialSystem_Config_t* config,
                    bool forceUpdate);
void BeginFrame(void* thisptr, float frameTime);

/* ModelRender */
void DrawModelExecute(void* thisptr, void* context, void* state,
                      const ModelRenderInfo_t& pInfo,
                      matrix3x4_t* pCustomBoneToWorld);

/* Panel */
void PaintTraverse(void* thisptr, VPANEL vgui_panel, bool force_repaint,
                   bool allow_force);

/* Sound */
void EmitSound2(void* thisptr, IRecipientFilter& filter, int iEntIndex,
                int iChannel, const char* pSoundEntry,
                unsigned int nSoundEntryHash, const char* pSample,
                float flVolume, int nSeed, soundlevel_t iSoundLevel, int iFlags,
                int iPitch, const Vector* pOrigin, const Vector* pDirection,
                void* pUtlVecOrigins, bool bUpdatePositions, float soundtime,
...
/* ViewRender */
void RenderView(void* thisptr, CViewSetup& setup, CViewSetup& hudViewSetup,
                unsigned int nClearFlags, int whatToDraw);
...
typedef void (*RenderViewFn)(void*, CViewSetup&, CViewSetup&, unsigned int,
                             int);