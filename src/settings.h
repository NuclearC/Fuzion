...
struct AimbotWeapon_t {
  bool enabled, silent, friendly, closestBone, engageLock, engageLockTR,
      aimkeyOnly, smoothEnabled, smoothSaltEnabled, errorMarginEnabled,
      autoAimEnabled, aimStepEnabled, rcsEnabled, rcsAlwaysOn,
      spreadLimitEnabled, autoPistolEnabled, autoShootEnabled, autoScopeEnabled,
      noShootEnabled, ignoreJumpEnabled, ignoreEnemyJumpEnabled, smokeCheck,
      flashCheck, autoWallEnabled, autoAimRealDistance, autoSlow, predEnabled,
      scopeControlEnabled;
  ...
  float smoothAmount = 1.0f, smoothSaltMultiplier = 0.0f,
        errorMarginValue = 0.0f, autoAimFov = 180.0f, aimStepMin = 25.0f,
        aimStepMax = 35.0f, rcsAmountX = 2.0f, rcsAmountY = 2.0f,
        autoWallValue = 10.0f, spreadLimit = 1.0f;
  ...
  bool operator==(const AimbotWeapon_t& another) const {
    for (int bone = (int)DesiredBones::BONE_PELVIS;
      ...
      if (this->desiredBones[bone] != another.desiredBones[bone]) return false;
    }

    return this->enabled == another.enabled && this->silent == another.silent &&
           ...
           this->bone == another.bone && this->aimkey == another.aimkey &&
...
class ColorVar {
 public:
  ...
  ImColor Color() {
    ImColor result =
        this->rainbow ? Util::GetRainbowColor(this->rainbowSpeed) : this->color;
...
class HealthColorVar : public ColorVar {
 public:
  ...
  ImColor Color(C_BasePlayer* player) {
    ImColor result =
        this->rainbow
            ? Util::GetRainbowColor(this->rainbowSpeed)
            : (this->hp ? Color::ToImColor(Util::GetHealthColor(player))
                        : this->color);
    result.Value.w = this->color.Value.w;
    return result;
  }
...
extern std::unordered_map<ItemDefinitionIndex, AimbotWeapon_t,
...
extern std::unordered_map<ItemDefinitionIndex, AttribItem_t,
                          Util::IntHash<ItemDefinitionIndex>>
    skinsCT;
extern std::unordered_map<ItemDefinitionIndex, AttribItem_t,
                          Util::IntHash<ItemDefinitionIndex>>
